          ...
          service.route(
            ...
            (result: any, status: any) => {
              ...
              if (status === 'OK' && result?.routes?.[0]?.legs?.[0]) {
                ...
                const fallbackKm = haversineKm(area.location, incident.location!);
                setRoutingGuidance('staging', area.id, {
                  status: 'ready',
                  destinationName: area.name,
                  summary: { distanceKm: fallbackKm, etaMinutes: (fallbackKm / 60) * 60 },
                  steps: [{ instruction: 'No road route found — straight-line distance shown.', distanceText: '', durationText: '' }]
                });
              } else {
                setRoutingGuidance('staging', area.id, {
                  status: 'error',
    ...
    service.route(
      ...
      (result: any, status: any) => {
        ...
        if (status === 'OK' && result?.routes?.[0]?.legs?.[0]) {
          ...
          const fallbackKm = haversineKm(incident.location!, nearestHospital.location);
          setRoutingGuidance('hospital', null, {
            status: 'ready',
            destinationName: nearestHospital.name,
            summary: { distanceKm: fallbackKm, etaMinutes: (fallbackKm / 60) * 60 },
